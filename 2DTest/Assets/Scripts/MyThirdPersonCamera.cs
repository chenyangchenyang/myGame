...
public class MyThirdPersonCamera : MonoBehaviour
{
	...
	void Start()
    {
		...
		deltaPostion = transform.position - follow.position;
	}
	...
	void LateUpdate ()
	{
		...
		//targetPosition = follow.position + Vector3.right * distanceUp - follow.forward * distanceAway;
		targetPosition= deltaPostion+follow.position;
		// making a smooth transition between it's current position and the position it wants to be in
		transform.position = Vector3.Lerp(transform.position, targetPosition, Time.deltaTime * smooth);

		// make sure the camera is looking the right way!
		transform.LookAt(transform.position - startPosition);
	}
}
